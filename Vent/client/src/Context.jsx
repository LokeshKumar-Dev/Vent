...
export function VentProvider({ children }) {
  ...
  const handleSidebar = useCallback(
    async (show, id, owner, network, verified, staffs) => {
      if (show) {
        ...
        if (staffs) {
          staffs.forEach((staff, i) => {
            if (isSameAddress(staff.staff, currentAddress)) {
              staffId = i;
            }
          });
        }

        console.log("handleSidebar", staffId, staffs, owner, currentAddress);
        setSidebar({
          ...sidebar,
          show,
          showId: id,
  ...
  const handleModal2 = useCallback(
    (open2, title2, vent2, staffId) => {
      console.log("modal", open2, title2, vent2);
      setModal({ ...modal, open2, title2, vent2, staffId });
    },
    [modal]
  );
  ...
  const handleSpendForm = async (
    ...
    if (receiver.length === 0 || !ethers.utils.isAddress(receiver)) {
      //Check address valid
      message.warning("Enter valid receiver");
      return;
    }
    if (name.length === 0) {
      message.warning("Enter reason");
      return;
    }
    if (fromCoin.length === 0) {
      message.warning("Select coin");
    ...
    //Check amount < balance
    const amount_wei = parseEtherToWei(amount);
    console.log("amount_wei", amount, amount_wei);

    contract.on("PaymentSuccessful", (from, amount) => {
      setLoading(false);
      handleCancle();
  ...
  async function addSponsor_GmpToken(
    ...
    try {
      ...
      setTransaction({ open: true, hash: transactionHash });
      setLoading(false);
