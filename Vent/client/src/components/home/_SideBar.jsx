        ...
        status.loading ? (
          ...
          Object.keys(vent).length > 0 && (
            <>
              ...
              {/* Pay Btn */}
              {sidebar?.staffId !== null && sidebar?.staffId > -1 && (
                <button
                  onClick={() =>
                    handleModal2(true, "Spend", vent, sidebar.staffId)
                  }
                  className="btn btn--primary"
                >
                  Spend {">>"}{" "}
              ...
              {isSameAddress(vent?.owner, currentAccount) && (
                <>
                  <Tooltip title={"copy to clipboard"}>
                    <button
                      onClick={() => navigator.clipboard.writeText(vent?._id)}
    ...
    <>
      <div className="amount">
        ...
        <h5>
          {parseFloat(
            formatEther(
              ethers.BigNumber.from(balance.toString()).sub(expense.toString())
            )
          ).toFixed(2)}{" "}
          <span>avax</span>
        </h5>
...
export function SideBarForOwner() {
  ...
  useEffect(() => {
    ...
    setStatus({ loading: true, error: false, message: "", _id: "" });
    // Vent get
    _getVent(
      Contract,
      sidebar.showId,
          ...
          Object.keys(vent).length > 0 && (
            <>
              ...
              {status._id && (
                <Tooltip title={"copy to clipboard"}>
                  <button
                    onClick={() => navigator.clipboard.writeText(status._id)}
                    style={{ fontSize: "1rem", marginBottom: ".1rem" }}
                    className="btn btn--secondary"
                  >
                    {status._id}
